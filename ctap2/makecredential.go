...
func (h *Handler) MakeCredential(ctx context.Context, req *MakeCredentialRequest) (byte, []byte) {
	...
	credIDSum := sha256.Sum256(credentialID)
	log.Printf("CTAP2 MakeCredential: credential ID SHA256=%x, rpIDHash=%x", credIDSum, rpIDHash)

	// Persist credential for RP to support cases where browser does not provide allowList
	if err := storage.SaveCredential(req.RP.ID, credentialID); err != nil {
		log.Printf("CTAP2 MakeCredential: failed to save credential: %v", err)
	} else {
		log.Printf("CTAP2 MakeCredential: saved credential for rp=%s", req.RP.ID)
	}
