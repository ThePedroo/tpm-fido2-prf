...
type Signer interface {
	...
	ValidateKeyHandle(keyHandle []byte) error
}
...
func (s *server) run() {
	...
	ctap2Handler := ctap2.NewHandler(s.signer, s.pe)
...
func (s *server) registerSite(ctx context.Context, token *fidohid.SoftToken, evt fidohid.AuthEvent) {
	...
	khSum := sha256.Sum256(keyHandle)
	log.Printf("Register site: keyHandle len=%d, keyHandle SHA256=%x", len(keyHandle), khSum)
